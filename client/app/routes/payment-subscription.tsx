...
import type { SubscriptionCard } from "../types/subscriptions";

type PaymentMethod = "card" | "paypal";
type PayFlowState = "idle" | "processing" | "success" | "error";

export default function PaymentSubscriptionRoute() {
  const { subscriptionId } = useParams();
  const { t } = useTranslation();
  const [subscription, setSubscription] = useState<SubscriptionCard | null>(null);
  ...
  useEffect(() => {
    ...
    const loadSubscription = async () => {
      ...
      const selected = data.items.find((item) => item.id === subscriptionId) ?? null;
  ...
  const handlePayAction = async () => {
    ...
    try {
      ...
      const order = await paymentsRepository.createPaypalOrder({
        ...
        amountLabel: subscription.priceLabel,
  ...
  if (!subscription) {
    return (
      <section className="w-screen min-h-[calc(100vh-64px)] bg-[#d4d4d4] ml-[calc(50%-50vw)] mr-[calc(50%-50vw)] p-6">
        {t("payments.subscription.loading")}
      </section>
    );
  }
    ...
    <section className="w-screen min-h-[calc(100vh-64px)] bg-[#d4d4d4] ml-[calc(50%-50vw)] mr-[calc(50%-50vw)]">
      <div className="w-full min-h-[calc(100vh-64px)] grid grid-cols-[minmax(0,1fr)_40%] border-x border-[#9a9a9a] max-[1000px]:grid-cols-1">
        <section className="bg-[#d4d4d4] px-8 py-9 border-r border-[#9a9a9a] max-[1000px]:border-r-0">
          <h1 className="m-0 text-[48px] leading-[1.1]">{t("payments.subscription.title")}</h1>
        ...
        <aside className="bg-[#f1f3f7] px-8 py-9">
          <h2 className="m-0 text-[42px] leading-[1.1]">{t("payments.subscription.level_title")}</h2>

          <div className="mt-3 flex items-center gap-3">
            <CircleUserRound size={48} />
            <div>
              <div className="text-[29px] leading-tight flex items-center gap-1.5">
                {t("payments.common.creator_name")}
                <CheckCircle2 size={18} />
              </div>
              <div className="text-[14px] leading-tight">{t("payments.common.creator_handle")}</div>
            </div>
          </div>

          <div className="mt-3 border-y border-[#8f8f8f] py-2 flex items-center justify-between text-[26px]">
            <span>{t("payments.subscription.monthly_payment")}</span>
            <span>{subscription.priceLabel}</span>
          </div>
          <div className="h-8" aria-hidden />

          {/* TODO(frontend-integration): enlazar este botón con la pasarela real de pagos para suscripciones. */}
          <button
            type="button"
