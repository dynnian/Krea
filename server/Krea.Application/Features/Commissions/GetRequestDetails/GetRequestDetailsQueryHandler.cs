...
{
    public async Task<CommissionRequestDto> Handle(GetRequestDetailsQuery request, CancellationToken cancellationToken)
    {
        var commissionRequest = await requestRepository.GetByIdWithAllAsync(request.RequestId, cancellationToken);
        if (commissionRequest == null)
            throw new Exception("Commission request not found.");

        var payments = commissionRequest.Payments.Select(p => new PaymentInfoDto(p.Id, p.Amount.Amount, p.Amount.Currency, p.Status.ToString(), p.PaidAt)).ToList();
        var submissions = commissionRequest.Submissions.Select(s => new SubmissionDto(
            s.Id,
            s.MediaId,
            s.Media.Path,
        ...
        return new CommissionRequestDto(
            ...
            new CommissionOfferingMinimalDto(commissionRequest.Offering.Id, commissionRequest.Offering.Title, commissionRequest.Offering.BasePrice.Amount, commissionRequest.Offering.BasePrice.Currency),
