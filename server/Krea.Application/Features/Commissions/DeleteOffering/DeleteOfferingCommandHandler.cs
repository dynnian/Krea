    ...
    public class DeleteOfferingCommandHandler(
        ...
        public async Task<Unit> Handle(DeleteOfferingCommand request, CancellationToken cancellationToken) {
            ...
            IReadOnlyList<CommissionRequest> requests =
                await requestRepository.GetByOfferingAsync(request.OfferingId, cancellationToken);
            if (requests.Any(r =>
                    r.Status != CommissionRequestStatus.Completed && r.Status != CommissionRequestStatus.Cancelled))
            ...
            logger.LogInformation("Offering {OfferingId} deleted by artist {ArtistId}", request.OfferingId,
                currentUserId);
            return Unit.Value;
        }
    }
