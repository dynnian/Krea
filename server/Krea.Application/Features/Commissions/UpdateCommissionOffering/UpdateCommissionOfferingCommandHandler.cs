    ...
    public class UpdateCommissionOfferingCommandHandler(
        ...
        public async Task<Unit> Handle(UpdateCommissionOfferingCommand request, CancellationToken cancellationToken) {
            ...
            CommissionOffering? offering =
                await offeringRepository.GetByIdForUpdateAsync(request.OfferingId, cancellationToken);
            if (offering == null)
            ...
            logger.LogInformation("Offering {OfferingId} updated by artist {ArtistId}", request.OfferingId,
                currentUserId);
            return Unit.Value;
        }
    }
