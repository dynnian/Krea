...
internal class MarkMessageAsReadCommandHandler(
    ...
    public async Task<bool> Handle(MarkMessageAsReadCommand request, CancellationToken cancellationToken)
    {
        ...
        Conversation? conversation = await conversationRepository.GetByIdAsync(message.Conversation.Id, cancellationToken);
        ConversationParticipant? participant = conversation?.Participants.FirstOrDefault(p => p.UserId == request.UserId);
        if (participant == null)
