        ...
        public async Task<Unit> Handle(ConfirmPaymentCommand request, CancellationToken cancellationToken)
    {
        ...
        switch (parentInfo.ParentType)
        {
            case PaymentParentType.CommissionRequest:
                CommissionRequest? commission = await commissionRepo.GetByIdWithPaymentsAsync(parentInfo.ParentId, cancellationToken);
            ...
            case PaymentParentType.Donation:
                Donation? donation = await donationRepo.GetByIdWithPaymentsAsync(parentInfo.ParentId, cancellationToken);
            ...
            case PaymentParentType.Subscription:
                Subscription? subscription = await subscriptionRepo.GetByIdWithPaymentsAsync(parentInfo.ParentId, cancellationToken);
