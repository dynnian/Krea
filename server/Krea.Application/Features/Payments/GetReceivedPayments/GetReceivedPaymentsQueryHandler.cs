...
public class GetReceivedPaymentsQueryHandler(
    ...
    ILogger<GetReceivedPaymentsQueryHandler> logger)
    : IRequestHandler<GetReceivedPaymentsQuery, PagedResult<PaymentSummaryDto>>
{
    public async Task<PagedResult<PaymentSummaryDto>> Handle(GetReceivedPaymentsQuery request, CancellationToken cancellationToken)
    {
