...
public class GetUserDonationsQueryHandler(
    ...
    ILogger<GetUserDonationsQueryHandler> logger)
    : IRequestHandler<GetUserDonationsQuery, PagedResult<DonationDto>>
{
    public async Task<PagedResult<DonationDto>> Handle(GetUserDonationsQuery request, CancellationToken cancellationToken)
    {
        var donations = await donationRepository.GetByDonorAsync(request.UserId, cancellationToken);
