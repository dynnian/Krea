...
namespace Krea.Infrastructure.Repositories {
    ...
    public sealed class CommissionOfferingRepository(AppDbContext context) : ICommissionOfferingRepository {
        public async Task<CommissionOffering?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
            await context.CommissionOfferings
                         .AsNoTracking()
                         .Include(o => o.Artist)
                         .FirstOrDefaultAsync(o => o.Id == id, cancellationToken);

        public async Task<IReadOnlyList<CommissionOffering>> GetByArtistAsync(
        ...
        public async Task<int> GetActiveRequestCountAsync(Guid offeringId,
                                                          CancellationToken cancellationToken = default) {
            var statuses = new[] {
                CommissionRequestStatus.Accepted, CommissionRequestStatus.InProgress, CommissionRequestStatus.Delivered
            };
            return await context.CommissionRequests
                                .CountAsync(cr => cr.Offering.Id == offeringId && statuses.Contains(cr.Status),
                                    cancellationToken);
        }
        ...
        public async Task<CommissionOffering?> GetByIdForUpdateAsync(
