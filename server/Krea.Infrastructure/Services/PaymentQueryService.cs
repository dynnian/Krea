namespace Krea.Infrastructure.Services {
    ...
    public class PaymentQueryService : IPaymentQueryService
    {
        ...
        public PaymentQueryService(AppDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<PaymentParentInfo?> GetParentByExternalRefAsync(ExternalPaymentRef externalRef, CancellationToken cancellationToken)
        {
            ...
            if (payment.CommissionRequestId.HasValue)
                return new PaymentParentInfo(payment.CommissionRequestId.Value, PaymentParentType.CommissionRequest, payment.Id);
            ...
            return payment.SubscriptionId.HasValue 
                ? new PaymentParentInfo(payment.SubscriptionId.Value, PaymentParentType.Subscription, payment.Id) : null;
        }
    }
}