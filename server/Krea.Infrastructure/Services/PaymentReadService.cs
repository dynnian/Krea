...
public class PaymentReadService : IPaymentReadService
{
    ...
    public PaymentReadService(AppDbContext context)
    {
        _context = context;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        IQueryable<PaymentEntity> query = _context.Payments
            ...
            .Where(p => (p.Donation != null && p.Donation.Recipient.Id == userId)
                        || (p.CommissionRequest != null && p.CommissionRequest.Offering.Artist.Id == userId)
                        || (p.Subscription != null && p.Subscription.Plan.Artist.Id == userId))
            .AsNoTracking();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var payerName = payment.Payer.DisplayName;
    ...
    {
        return query.OrderByDescending(p => p.PaidAt ?? p.CreatedAt);
    }
