...
public sealed class FakeSender : ISender {
    ...
    public async Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default) {
        if (!_handlers.TryGetValue(request.GetType(), out var handler)) {
            throw new InvalidOperationException($"No fake handler configured for request type {request.GetType().Name}.");
        }
        ...
        throw new InvalidOperationException($"Configured fake handler returned incompatible result for {request.GetType().Name}.");
    }
}
