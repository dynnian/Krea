namespace Krea.API.Tests.TestSupport {
    ...
    public static class TestDataSeeder {
        ...
        public static async Task<SeededUsers> SeedBasicUsersAsync(IServiceProvider services) {
            ...
            await CreateIdentityUserAsync(userManager, adminDomain.Id, "admin", "admin@test.local", "Admin123!",
                "Admin");
            await CreateIdentityUserAsync(userManager, artistDomain.Id, "artist", "artist@test.local", "Admin123!",
                "Artist");
            await CreateIdentityUserAsync(userManager, otherDomain.Id, "other", "other@test.local", "Admin123!",
                "Artist");
        ...
        public static async Task<Guid> SeedPostAsync(IServiceProvider services, Guid authorId,
                                                     string title = "Hello post") {
            var db = services.GetRequiredService<AppDbContext>();
        ...
        private static async Task CreateIdentityUserAsync(
            ...
            string password,
            string role) {
            var appUser = new AppUser { Id = userId, UserName = userName, Email = email, EmailConfirmed = true };

            IdentityResult createResult = await userManager.CreateAsync(appUser, password);
            if (!createResult.Succeeded) {
                throw new InvalidOperationException(
                    $"Unable to create user '{userName}': {string.Join(", ", createResult.Errors.Select(e => e.Description))}");
            }

            IdentityResult roleResult = await userManager.AddToRoleAsync(appUser, role);
            if (!roleResult.Succeeded) {
                throw new InvalidOperationException(
                    $"Unable to add role '{role}' to '{userName}': {string.Join(", ", roleResult.Errors.Select(e => e.Description))}");
            }
        }
    }
}