namespace Krea.API.Tests.Integration {
    ...
    public sealed class AdminControllerIntegrationTests(PostgresContainerFixture postgres) {
        ...
        public async Task GetPostModerationReports_ReturnsSeededPendingReport() {
            ...
            await using var host = await IntegrationTestHost.CreateAsync(_postgres,
                seed: async services => {
                    seeded = await TestDataSeeder.SeedBasicUsersAsync(services);
                    Guid postId =
                        await TestDataSeeder.SeedPostAsync(services, seeded.ArtistId, "post for moderation list");
