...
public sealed class CollectionsControllerIntegrationTests {
    ...
    public CollectionsControllerIntegrationTests(PostgresContainerFixture postgres) {
        _postgres = postgres;
    }
    ...
    public async Task AddPostAndRemovePost_WorksForCollection() {
        ...
        using var removeRequest = IntegrationTestHost.CreateAuthenticatedRequest(
            HttpMethod.Delete,
        ...
        int countAfterRemove =
            await removeDb.Collections.Where(c => c.Id == collectionId).Select(c => c.ItemCount).SingleAsync();
    ...
    private static async Task<Guid> CreateCollectionAsync(IntegrationTestHost host, Guid userId) {
        ...
        HttpResponseMessage createResponse = await host.Client.SendAsync(request);
        createResponse.EnsureSuccessStatusCode();

        using JsonDocument json = JsonDocument.Parse(await createResponse.Content.ReadAsStringAsync());
