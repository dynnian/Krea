...
public sealed class CollectionsControllerIntegrationTests
{
    ...
    public async Task CreateCollection_ReturnsCreatedAndPersistsCollection()
    {
        ...
        using var form = BuildCreateCollectionForm("My Collection", "Integration description", 0);
    ...
    public async Task GetUserCollections_ReturnsCollectionsForUser()
    {
        ...
        using var json = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        Assert.Contains(json.RootElement.EnumerateArray(), item => item.GetProperty("id").GetGuid() == collectionId);
    }
    ...
    public async Task AddPostAndRemovePost_WorksForCollection()
    {
        ...
        var addResponse = await IntegrationTestHost.SendAuthenticatedAsync(
            host.Client,
            HttpMethod.Post,
        ...
        using var addScope = host.App.Services.CreateScope();
        var addDb = addScope.ServiceProvider.GetRequiredService<AppDbContext>();
        int countAfterAdd = await addDb.Collections.Where(c => c.Id == collectionId).Select(c => c.ItemCount).SingleAsync();
        Assert.Equal(1, countAfterAdd);

        using var removeRequest = CreateAuthenticatedRequest(
            HttpMethod.Delete,
            $"/api/collections/{collectionId}/posts/{postId}",
            seeded.AdminId,
            "Artist");

        var removeResponse = await host.Client.SendAsync(removeRequest);
        Assert.Equal(HttpStatusCode.NoContent, removeResponse.StatusCode);

        using var removeScope = host.App.Services.CreateScope();
        var removeDb = removeScope.ServiceProvider.GetRequiredService<AppDbContext>();
        int countAfterRemove = await removeDb.Collections.Where(c => c.Id == collectionId).Select(c => c.ItemCount).SingleAsync();
    ...
    public async Task UploadCover_WithValidFile_ReturnsOk()
    {
        ...
        using var json = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        Assert.True(json.RootElement.TryGetProperty("mediaId", out var mediaId));
        Assert.True(json.RootElement.TryGetProperty("url", out var url));
    ...
    {
        using var form = BuildCreateCollectionForm("Seed collection", "Seed desc", 0);
        using var request = CreateAuthenticatedRequest(
            HttpMethod.Post,
            "/api/collections",
            userId,
