namespace Krea.API {
    ...
    internal static class Program {
        ...
        public static async Task Main(string[] args) {
            ...
            builder.Configuration.AddJsonFile("/data/appsettings.json", true, true);
        ...
        private static void CopyProxyResponseHeaders(HttpResponseMessage source, HttpResponse target) {
            foreach ((string key, IEnumerable<string> value) in source.Headers) {
                if (ShouldSkipProxyResponseHeader(key)) { continue; }
        ...
        private static string[] ResolveAllowedOrigins(IConfiguration configuration, bool isDevelopment) {
            ...
            if (!string.IsNullOrWhiteSpace(publicUrl)) {
                if (!publicUrl.StartsWith("http://", StringComparison.OrdinalIgnoreCase) &&
                    ...
                    if (uri.Scheme == Uri.UriSchemeHttps) { configuredOrigins.Add($"http://{uri.Authority}"); }
                    else if (uri.Scheme == Uri.UriSchemeHttp) { configuredOrigins.Add($"https://{uri.Authority}"); }
                }
            }
        ...
        private static string ResolveBooleanConfiguration(string? rawValue, bool fallback) {
            if (string.IsNullOrWhiteSpace(rawValue)) { return fallback ? "true" : "false"; }

            if (rawValue == "1") { return "true"; }

            if (rawValue == "0") { return "false"; }

            return bool.TryParse(rawValue, out bool parsed) ? parsed ? "true" : "false" : fallback ? "true" : "false";
        }
        ...
        private static string NormalizeMinioEndpoint(string endpoint) {
            ...
            if (!Uri.TryCreate(candidate, UriKind.Absolute, out Uri? uri)) { return candidate.TrimEnd('/'); }
