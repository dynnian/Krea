...
#pragma warning restore CS8618
        ...
        public static Conversation CreateDirectMessage() => new(ConversationType.DirectMessage, null, null, null);

        public static Conversation CreateGroup(string title, string? description, Media? icon = null) =>
            new(ConversationType.Group, title, description, icon);
        ...
        public void RemoveParticipant(Guid userId) {
            ConversationParticipant? participant = _participants.FirstOrDefault(p => p.UserId == userId);
        ...
        public Message SendTextMessage(User sender, string text) {
            ...
            void IncrementUnreadForOthers(Guid senderId) {
                foreach (ConversationParticipant p in _participants.Where(p => p.UserId != senderId && p.IsActive))
                    p.IncrementUnread();
            }
        }
